function buscar_fact() {
    ...
    var base_url =
        window.location.origin + "/anitacafe/index.php/pagos/buscar_fact_ind";
    $.ajax({
        url: base_url,
        method: "post",
            ...
            if (data["id_estatus"] <= 4 || data["id_estatus"] == 6) {
                ...
                if (id_registro_fact) {
                    var base_url =
                        ...
                        "/anitacafe/index.php/pagos/buscar_fact_art";
                    $.ajax({
                        url: base_url,
                        method: "post",
                        ...
                        success: function(data) {
                            $.each(data, function(index, response) {
                                $("#data-table-responsive tbody").append(
                                    "<tr><td>" +
                                    response["id_reg_prod_fact"] +
                                    "</td><td>" +
                                    response["nom_producto"] +
                                    "</td><td>" +
                                    response["categoria"] +
                                    "</td><td>" +
                                    response["cantidad"] +
                ...
                if (id_estatus >= "8") {
                    ...
                    var base_url =
                        window.location.origin +
                        "/anitacafe/index.php/pagos/buscar_mov_fac";
                    $.ajax({
                        url: base_url,
                        method: "post",
                        data: { id_registro_fact: id_registro_fact },
                        ...
                        success: function(data) {
                            $.each(data, function(index, response) {
                                $("#pruebaa").append(
                                    "<tr><td>" +
                                    response["id_registro_fact"] +
                                    "</td><td>" +
                                    response["total_ant_bs"] +
                                    "</td><td>" +
                                    response["id_moneda"] +
                                    "</td><td>" +
                                    response["valor"] +
                                    ...
                                    response["total_om"] +
                                    "</td><td>" +
                                    response["total_abonado_bs"] +
                                    "</td><td>" +
                                    response["total_abonado_om"] +
                                    "</td><td>" +
                                    response["restante_bs"] +
                                    "</td><td>" +
                                    response["restante_om"] +
                                    "</td><td>" +
                                    response["fecha_reg"] +
                                    "</td></tr>"
                                );
...
function llenar_pago() {
    ...
    if (tipo_pago != "1") {
        ...
        $("#monto_om").attr("readonly", true);
    }
}

function act() {
    $("#data-table-responsive tbody").load(
        location.href + "#data-table-responsive tbody>*",
        ""
    );
}

function modal(id) {
    var id_reg_factura = id;
    console.log(id_reg_factura);

    var base_url =
        window.location.origin + "/anitacafe/index.php/Pagos/consultar_total_fact";
    ...
    swal
        ...
        .then((result) => {
            if (result.value == true) {
                ...
                var base_url =
                    window.location.origin +
                    "/anitacafe/index.php/Pagos/guardar_proc_pag";
                var base_url_2 =
                    window.location.origin + "/anitacafe/index.php/Pagos/ingreso";
                $.ajax({
                    url: base_url,
                    method: "POST",
                    data: datos,
